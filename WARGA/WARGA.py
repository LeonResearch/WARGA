...
def gae_for(args):
    ...
    for epoch in range(args.epochs):
        ...
        # Generate embeddings
        recovered, emb = model(features, adj_norm)
        
        # Wasserstein Regularizer
        for i in range(1):
            d_fake = regularizer(emb).to(device)
            z_real_dist = torch.normal(0.0, 1.0, [n_nodes, args.hidden2]).to(device)
            d_real = regularizer(z_real_dist)          
            
            dc_loss = - d_real.mean() + d_fake.mean() 
        ...
        # Update Generator
        d_fake = regularizer(emb)  
        generator_loss = -d_fake.mean()        
        loss = loss_function1(preds=recovered, labels=adj_label,
